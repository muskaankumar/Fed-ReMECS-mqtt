...
def arousal_model(global_weight_emg,global_weight_eog,global_weight_gsr):
  sequence_length = 128
  num_features0 = 2
  ...
  # out = Dropout(0.25)(out)
  out = Dense(2, activation='softmax')(out)
  MDCARE_arousal = Model(inputs=[encoder_eog.input, encoder_emg.input, encoder_gsr.input], outputs=out)

  MDCARE_arousal.compile(optimizer=Adam(learning_rate=0.0001, decay=1e-6), 
                loss='categorical_crossentropy', 
                metrics=['accuracy'])
  
  return MDCARE_arousal
