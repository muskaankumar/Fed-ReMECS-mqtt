...
topic_list =[('LocalModel_for_EMG_Signal',0),('LocalModel_for_EOG_Signal',0),('LocalModel_for_GSR_Signal',0),('ModelPerformance',0),('LocalModel_for_Valence',0),('ModelPerformance_valence',0)]
...
while True:
    ...
    while not qGSModel_emg.empty():
        ...
        local_model_weights_emg = list(decodedweights)
    ...
    if i >0:
        ...
        client.publish("GlobalModel_EMG", payload = encodedGlobalModelWeights) #str(Global_weights), qos=0, retain=False)
...
#         break
    ...
    while not qGSModel_eog.empty():
        message = qGSModel_eog.get()
        ...
        # Convert object to a list
        local_model_weights_eog = list(decodedweights)
    ...
    if i >0:
        ...
        client.publish("GlobalModel_EOG", payload = encodedGlobalModelWeights) #str(Global_weights), qos=0, retain=False)
        print("Broadcasted Global Model to Topic:--> GlobalModel")
...
#         break
    ...
    while not qGSModel_gsr.empty():
        message = qGSModel_gsr.get()
        ...
        # Convert object to a list
        local_model_weights_gsr = list(decodedweights)
        scaled_weights = scale_model_weights(local_model_weights_gsr, 0.1)
        all_local_model_weights_gsr.append(scaled_weights)

    print('Total Local Model Received for GSR :',len(all_local_model_weights_gsr))
    ...
    if i >0:
        ...
        client.publish("GlobalModel_GSR", payload = encodedGlobalModelWeights) #str(Global_weights), qos=0, retain=False)
        print("Broadcasted Global Model to Topic:--> GlobalModel")
...
while True:
    ...
    if (i>0): #after first round of model exchange global models performance is calculated
        ...
        if (len_local_perfm != 0):
            ...
            print('----------------------------------------------------')
            print('Classification Report  for Valence:',global_performance[1])
    ...
    while not qGSValence.empty():
        ...
        # Convert object to a list
        local_model_weights_valence = list(decodedweights)
    ...
    if i >0:
        ...
        client.publish("GlobalModel_Valence", payload = encodedGlobalModelWeights) #str(Global_weights), qos=0, retain=False)
